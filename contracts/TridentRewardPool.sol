...
contract TridentRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accTridentPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00434 ether; // 45000 TRIDENT / (120 days * 24h * 60min * 60s)
    uint256 public runningTime = 120 days;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "TridentRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "TridentRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending Trident on frontend.
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _tridentReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accTridentPerShare = accTridentPerShare.add(_tridentReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accTridentPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _tridentReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accTridentPerShare = pool.accTridentPerShare.add(_tridentReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accTridentPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeTridentTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accTridentPerShare).div(1e18);
    ...
    function safeTridentTransfer(address _to, uint256 _amount) internal {
        uint256 _tridentBal = trident.balanceOf(address(this));
        if (_tridentBal > 0) {
            if (_amount > _tridentBal) {
                trident.safeTransfer(_to, _tridentBal);
            } else {
                trident.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            ...
            require(_token != trident, "trident");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
